...
def summarize_tool_input(tool_call: ToolCall, file_state: AgentFileState) -> Dict[str, Any]:
    """Compact, UI-friendly view of a tool call's arguments (sent as ``toolStart.input``)."""
    return summarize_tool_args(tool_call.name, tool_call.arguments or {}, file_state)


def summarize_tool_args(
    name: str, args: Dict[str, Any], file_state: AgentFileState
) -> Dict[str, Any]:
    tool_call = ToolCall(id="", name=name, arguments=args)
