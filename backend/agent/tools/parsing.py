...
def _extract_partial_json_string(raw_text: str, key: str) -> Optional[str]:
    ...
    start = cursor + 1
    # In a valid JSON prefix the first unescaped quote ends the string; if it
    # has not arrived yet the string is still streaming and we take the rest.
    end_quote: Optional[int] = None
    cursor = start
    ...
    partial = raw_text[start:] if end_quote is None else raw_text[start:end_quote]
