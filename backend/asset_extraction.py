...
from config import ASSET_EXTRACTION_MODEL, OPENROUTER_APP_TITLE, OPENROUTER_APP_URL, OPENROUTER_BASE_URL


# Reached through OpenRouter; override with ASSET_EXTRACTION_MODEL.
ASSET_EXTRACTION_GEMINI_MODEL = ASSET_EXTRACTION_MODEL
MAX_ASSETS_PER_GEMINI_REQUEST = 25
# OpenRouter image detail. "original" asks for full-resolution input (an
# OpenRouter extension, downgraded to "high" where unsupported); it replaces
# the Gemini-native MEDIA_RESOLUTION_ULTRA_HIGH setting of the old client.
...
class AssetExtractionMetrics:
    ...
    def record_response(self, response: Any, latency_seconds: float) -> None:
        """Record one detection call. ``response`` is the raw LangChain AIMessage."""
...
def build_detector(api_key: str, *, model: str | None = None) -> Any:
    """A structured-output runnable that returns ``{"raw": AIMessage, "parsed": ...}``.

    Uses OpenRouter's ``response_format: json_schema`` (supported on the Gemini
    endpoints) with ``require_parameters`` so the request only routes to
...
async def _locate_asset_batch(
    ...
    *,
    ...
    # Keep the task prompt after all image parts, as recommended for image
    # understanding. The prompt maps attachment order back to the caller's
    # original 1-based source image numbers.
...
async def extract_assets_from_images(
    ...
    *,
    ...
    """Locate and crop the described assets from the input screenshots.

    ``api_key`` is the OpenRouter key. ``detector`` lets tests (and the
    benchmark) inject a pre-built structured-output runnable.
    """
    ...
    # Batches are independent, so requests above the 25-object cap can run in
    ...
    batch_results = await asyncio.gather(
        *(
            _locate_asset_batch(
                detector, source_images, chunk, metrics, image_detail=image_detail
            )
