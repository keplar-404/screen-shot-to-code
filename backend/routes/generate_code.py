...
from models.factory import MissingOpenRouterKeyError
from routes.model_choice_sets import select_variant_models
...
class ParameterExtractionStage:
    ...
    async def extract_and_validate(self, params: Dict[str, Any]) -> ExtractedParams:
        ...
        # One key for every model: OpenRouter. The settings dialog value wins
        ...
        # Base URL for the OpenRouter API (proxies). Disabled in prod.
        openrouter_base_url: str | None = None
        if not IS_PROD:
            openrouter_base_url = self._get_from_settings_dialog_or_env(
                params, "openRouterBaseURL", OPENROUTER_BASE_URL
            )
...
class ModelSelectionStage:
    """Selects the variant models. Everything runs through OpenRouter, so the
    only prerequisite is an OpenRouter key (or the scripted fake model)."""
    ...
    async def select_models(
        ...
        if not openrouter_api_key and not S2C_FAKE_MODEL:
            message = str(MissingOpenRouterKeyError())
        ...
        num_variants = (
            NUM_VARIANTS_VIDEO
            if input_mode == "video"
            else 2 if generation_type == "update" else NUM_VARIANTS
        )
        variant_models = select_variant_models(generation_type, input_mode, num_variants)
...
def describe_variant_error(error: BaseException) -> str:
    """User-facing text for a failed variant.

    OpenRouter errors arrive as SDK exceptions carrying the HTTP status; map
    the common ones to actionable messages (the raw text is still logged).
    """
    status = getattr(error, "status_code", None)
    text = str(error)
    lowered = text.lower()
    if status == 401 or "user not found" in lowered or "no auth credentials" in lowered:
        return (
            "Incorrect OpenRouter key. Please make sure your OpenRouter API key is "
            "correct (https://openrouter.ai/keys)."
        )
    if status == 402 or "insufficient credits" in lowered:
        return "OpenRouter error - insufficient credits. Please top up your OpenRouter account."
    if status == 429:
        return "OpenRouter error - rate limited. Please wait a moment and try again."
    if status == 404:
        return f"OpenRouter could not find the requested model: {text}"
    return text
...
class AgenticGenerationStage:
    ...
    async def _run_variant(
        ...
        try:
            ...
            return completion
        except MissingOpenRouterKeyError as e:
            print(f"[VARIANT {index + 1}] Missing OpenRouter key", e)
            ...
            await self.send_message(
                "variantError", describe_variant_error(e), index, None, None
            )
            return ""
    ...
    async def process(
        ...
        try:
            ...
            generation_stage = AgenticGenerationStage(
                ...
                openrouter_api_key=context.extracted_params.openrouter_api_key,
                openrouter_base_url=context.extracted_params.openrouter_base_url,
