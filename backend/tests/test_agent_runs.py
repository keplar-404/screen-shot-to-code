...
async def test_disabled_recorder_writes_nothing(logs_path: Path) -> None:
    ...
    recorder.record_llm_request("openrouter", "openai/gpt-5.5", {"input": []})
...
async def test_full_run_records_events_costs_and_totals(logs_path: Path) -> None:
    ...
    recorder.record_llm_request("openrouter", "openai/gpt-5.5", {"input": ["go"]})
    ...
    recorder.record_llm_request("openrouter", "openai/gpt-5.5", {"input": ["continue"]})
    recorder.record_stream_event(
    ...
    pricing = MODEL_PRICING["openai/gpt-5.5"]
...
def _make_runner(recorder: AgentRunRecorder, model: Any) -> Any:
...
def _tool_then_done_turns(cost: Optional[float] = None) -> list[dict[str, Any]]:
    turns: list[dict[str, Any]] = [
        {
            "thinking": "planning",
            "tool": "create_file",
            "id": "call_1",
            "args": {"path": "index.html", "content": "<html>from tool</html>"},
        },
        {"text": "done"},
    ]
    if cost is not None:
...
async def test_engine_records_full_run(
    ...
    model = ScriptedModel(turns=_tool_then_done_turns(cost=0.01))
    monkeypatch.setattr("agent.deep_runner.make_chat_model", lambda *a, **k: model)

    recorder = _make_recorder()
    runner = _make_runner(recorder, model)
    result = await runner.run(
        Llm.GPT_5_5_HIGH, [{"role": "system", "content": "sys"}, {"role": "user", "content": "go"}]
    )
    ...
    assert types.count("llm_call_start") == 2
    assert types.count("llm_call_end") == 2
    assert "tool_call_start" in types and "tool_call_end" in types
    assert any(e["type"] == "stream_delta" and e.get("kind") == "thinking" for e in events)
    assert any(e["type"] == "stream_delta" and e.get("kind") == "tool_call" for e in events)
    run_end = events[-1]
    assert run_end["status"] == "completed"
    assert run_end["num_llm_calls"] == 2
    assert run_end["num_tool_calls"] == 1
    assert (Path(recorder.run_dir) / "final.html").read_text() == "<html>from tool</html>"
...
async def test_engine_aborts_over_budget_run(
    ...
    from agent.middleware import BudgetExceededError
    from tests.scripted_model import ScriptedModel

    model = ScriptedModel(turns=_tool_then_done_turns(cost=5.0))
    monkeypatch.setattr("agent.deep_runner.make_chat_model", lambda *a, **k: model)

    recorder = _make_recorder()
    runner = _make_runner(recorder, model)
    with pytest.raises(BudgetExceededError) as excinfo:
        await runner.run(Llm.GPT_5_5_HIGH, [{"role": "system", "content": "sys"}, {"role": "user", "content": "go"}])
...
async def test_engine_allows_final_turn_over_budget(
    ...
    turns = _tool_then_done_turns()
    turns[0]["cost"] = 0.5
    turns[1]["cost"] = 5.0  # crosses the ceiling only on the final (no-tool) turn
    model = ScriptedModel(turns=turns)
    monkeypatch.setattr("agent.deep_runner.make_chat_model", lambda *a, **k: model)
    recorder = _make_recorder()
    runner = _make_runner(recorder, model)
    result = await runner.run(Llm.GPT_5_5_HIGH, [{"role": "system", "content": "sys"}, {"role": "user", "content": "go"}])
    assert result == "<html>from tool</html>"
...
async def test_engine_fails_empty_output_runs(
    ...
    from agent.deep_runner import EmptyOutputError
    from tests.scripted_model import ScriptedModel

    # The observed gemini-3.6-flash failure mode: a final turn with no tool
    # calls and no text at all.
    model = ScriptedModel(turns=[{"text": ""}])
    monkeypatch.setattr("agent.deep_runner.make_chat_model", lambda *a, **k: model)
    recorder = _make_recorder()
    runner = _make_runner(recorder, model)
...
async def test_engine_records_failed_run_and_reraises(
    ...
    class _ExplodingModel(ScriptedModel):
        def _chunks(self, turn: dict) -> Any:
            raise RuntimeError("provider exploded")

    model = _ExplodingModel(turns=[{"text": "x"}])
    monkeypatch.setattr("agent.deep_runner.make_chat_model", lambda *a, **k: model)
...
async def test_list_and_detail_routes(logs_path: Path) -> None:
    ...
    recorder_c.record_llm_request("openrouter", "openai/gpt-5.5", {})
...
def test_recorder_survives_internal_errors(logs_path: Path) -> None:
    ...
    recorder.record_llm_request("openrouter", "openai/gpt-5.5", {"obj": object()})
