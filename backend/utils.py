...
def pprint_prompt(prompt_messages: List[PromptMessage]):
    print(json.dumps(truncate_data_strings(prompt_messages), indent=4))


def format_prompt_summary(prompt_messages: List[PromptMessage], truncate: bool = True) -> str:
    parts: list[str] = []
...
def print_prompt_summary(prompt_messages: List[PromptMessage], truncate: bool = True):
...
def print_prompt_preview(prompt_messages: List[PromptMessage]) -> None:
...
def truncate_data_strings(data: List[PromptMessage]):  # type: ignore
