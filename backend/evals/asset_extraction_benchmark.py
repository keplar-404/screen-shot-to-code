"""Opt-in live benchmark for the asset-extraction pipeline (via OpenRouter).

Run from ``backend`` so no API traffic happens accidentally::

    poetry run python -m evals.asset_extraction_benchmark --live

The report compares the batched structured pipeline at OpenRouter image
detail ``high`` and ``original`` (the closest equivalents of Gemini's
HIGH / ULTRA_HIGH media resolution). It writes JSON, an HTML summary, crops,
and box overlays under ``evals_data/asset_extraction_benchmark``.
"""
...
def _profile_settings(name: str) -> dict[str, Any]:
    detail = "high" if name == "new_high" else "original"
    return {
        "batch_size": 25,
        "response_schema": True,
        "temperature": 0.5,
        "thinking": "minimal",
        "media_resolution": f"OpenRouter image detail={detail}",
    }
...
async def run_benchmark(
    *,
    ...
    print("running new_high (detail=high)...", flush=True)
    profile_runs.append(("new_high", await _run_new_profile(fixture, api_key, "high")))
    print("running new_ultra_high (detail=original)...", flush=True)
    profile_runs.append(
        ("new_ultra_high", await _run_new_profile(fixture, api_key, "original"))
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the opt-in live asset-extraction fixture benchmark (OpenRouter)."
    )
    parser.add_argument(
        "--live",
        action="store_true",
        help="Required acknowledgement that this command makes paid OpenRouter API calls.",
    )
