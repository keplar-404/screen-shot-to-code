...
- Always use the backend Poetry virtualenv (Python 3.11+, `.venv` inside `backend/`) for Python commands.
...
- `pyproject.toml` requires Python >=3.11,<3.14 (LangChain 1.x / Deep Agents need 3.11+). Just use `poetry run`.
- Core feature (screenshot → code) requires one key: set `OPENROUTER_API_KEY` in `backend/.env` (restart backend after editing) or in the in-app Settings dialog. All models (OpenAI, Anthropic, Gemini) are called through OpenRouter via LangChain Deep Agents. Without a key, generation fails fast with a "No OpenRouter API key found" message. `S2C_FAKE_MODEL=1` runs a scripted local model for UI/dev work without any key. `REPLICATE_API_KEY` is optional and only powers `remove_backgrounds`.
